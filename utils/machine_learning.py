...
class SimpleLstm():
    ...
    def load_data(self, raw_data, sequence_length, dependent_var_col=1):
        """
        Loads the data
        ...
        d = dependent_var_col  ## closing price
        ...
        start = int(np.round(dr.shape[0] * 0.6))
        end = int(dr.shape[0] + 1)

        # print(d0.shape)
        unnormalized_bases = d0[start:end, 0:1, c]  # 1 on z is the closeing price
    ...
    def initialize_model(self, window_size, dropout_value, activation_function, loss_function, optimizer):
        """
        ...
        model.add(
            Bidirectional(LSTM(window_size, return_sequences=True), input_shape=(window_size, X_train.shape[-1]), ))
    ...
    def test_model(self, model, X_test, Y_test, unnormalized_bases):
        """
    ...
    def binary_price(self, delta_predict, delta_real):
        """
    ...
    def find_positives_negatives(self, delta_predict_1_0, delta_real_1_0):
        """
    ...
    def calculate_statistics(self, true_pos, false_pos, true_neg, false_neg, y_predict, Y_test):
        """
